...
public class RuleExpressiondimEntity_HI_RO  implements Comparable{
    public static final String query="SELECT   rd.rule_dim_reference_code      AS ruleDimReferenceCode,   rd.rule_dim_reference_from      AS ruleDimReferenceFrom,   rd.rule_DIM_value_from          AS ruleDimValueFrom,   rd.placeholder   AS placeholder,  re.RULE_EXP_DIM_ID AS ruleExpDimId,   rd.RULE_DIM_ID                  AS ruleDimId,   re.rule_business_line_code AS ruleBusinessLineCode,   rbl.rule_business_line_name AS ruleBusinessLineName,   re.rule_order_Id AS ruleOrderId,   rd.RULE_DIM_CODE AS ruleDimCode,   rd.RULE_DIM_NAME AS ruleDimName,   rd.RULE_DIM_DATA_TYPE AS ruleDimDataType,   re.rule_dim_operators AS ruleDimOperators,   re.rule_dim_value AS ruleDimValue,   re.RULE_EXP_CODE AS ruleExpCode,   re.EFFECT_DATE AS effectDate,   re.EFFECT_END_DATE AS effectEndDate,   rbl.rule_business_line_mappType AS ruleBusinessLineMapptype  FROM rule_expressiondim re LEFT JOIN rule_business_line rbl ON rbl.rule_business_line_code = re.rule_business_line_code   LEFT JOIN rule_dim rd ON rd.RULE_DIM_CODE=re.RULE_DIM_CODE AND rd.rule_business_line_code=re.rule_business_line_code WHERE 1=1";
